 ...
 *
 *   3) Based on the running times observed above, we can conclude that the experiment results
 *      successfully match what is expected. For the brute force algorithm, running time increases
 *      approximately as a function of n as O(n^2) or quadratically. This is supported by the fact that as n increased by a factor of 10,
 *      the run time increased by a factor of 100 (as n grows by one digit, run time grows by two digits). when n increases from 10000 to 100000, the run time
 *      increases from ~43 million to ~ 3.2 billion nanoseconds. The divide and conquer algorithm is much more efficient, as run time increases approximately as a function of n as O(n log n).  
 *      This is supported by the data where each time n increases by a factor of 10, run time also increases; and as n gets larger, the amount of growth in time increases but it is not
 *      exponential growth such as in brute force.
 *		The most efficient algorithm, however, is the Kadane algorithm; where we see running time grow approximately in a linear function of n, O(n). When observing the run times of n=10^4  
 *		through n=10^6, we notice that the growth is 
 *      rather steady, approximately 2 million nanoseconds each time. More support of this being linear growth is the fact that the run time for larger inputs such as n=10^6 are still 
 *      drastically smaller than that of the brute force and divide and conquer methods.
 *      
 *
